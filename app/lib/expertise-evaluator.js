...
export const expertiseEvaluationSchema={
  type:'object',additionalProperties:false,
  properties:{evaluations:{type:'array',minItems:1,maxItems:25,items:{type:'object',additionalProperties:false,properties:{
...
- For MATCHED, TRANSFERABLE, or PARTIAL, provide 1-3 short verbatim excerpts copied exactly from the Source CV. Do not paraphrase evidence.
...
export function validateExpertiseEvaluations(value,requirements=[],sourceCv=''){
  ...
  const seen=new Set()
  const normalizedCv=normalizeEvidenceText(sourceCv)
...
export async function evaluateExpertiseSemantically(requirements=[],sourceCv='',modelCall){
  ...
  const result=await callStructuredAi({
    ...
    schema:expertiseEvaluationSchema,
