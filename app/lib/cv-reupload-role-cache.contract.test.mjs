...
test('successful CV replace clears stale role draft before the next analysis',()=>{
  ...
  const nonPrimaryDraftClear=source.indexOf('if(slot!==1) setDraft(current=>resetRoleDraft(current))',libraryState)
  const primaryDraftClear=source.indexOf('setDraft(current=>resetRoleDraft(resumeToProfile(current,primaryCv)))',libraryState)
  const roleStateReset=source.indexOf('setProfileRoleState(EMPTY_ROLE_STATE)',libraryState)
  assert.ok(parseStart>=0)
  assert.ok(libraryState>parseStart)
  assert.ok(nonPrimaryDraftClear>libraryState)
  assert.ok(primaryDraftClear>libraryState)
  assert.ok(roleStateReset>libraryState)
})
