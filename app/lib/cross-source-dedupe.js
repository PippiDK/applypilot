...
export function isHighConfidenceDuplicate(a,b){
  if(!crossSource(a,b)) return false
  const aUrl=urlKey(a?.applicationUrl),bUrl=urlKey(b?.applicationUrl)
...
function sourceIdentity(record){return [record?.source,record?.sourceJobId,record?.detailUrl,record?.applicationUrl].map(value=>String(value??'').trim().toLowerCase()).join('|')}
function nonEmpty(value){return value!==null&&value!==undefined&&String(value).trim()!==''}

function mergeJobs(a,b){
  const richer=(String(b?.fullJd||'').length>String(a?.fullJd||'').length)?b:a
  const poorer=richer===a?b:a
  const merged={...poorer,...richer}
  for(const [key,value] of Object.entries(poorer||{})) if(!nonEmpty(merged[key])&&nonEmpty(value)) merged[key]=value
  const records=[]
  const seen=new Set()
...
export function dedupeJobs(jobs=[]){
  ...
  for(const raw of Array.isArray(jobs)?jobs:[]){
    ...
    const index=output.findIndex(existing=>isHighConfidenceDuplicate(existing,job))
    if(index<0) output.push(job)
    else output[index]=mergeJobs(output[index],job)
  }
  return output
}
